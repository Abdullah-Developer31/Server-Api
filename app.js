...
document.getElementById('addUser').addEventListener('click', async event => {
  ...
  axios.post('https://crud-server-api-nodejs.herokuapp.com/user', {
    "name": name,
    "email": email,
    "address": address,
...
document.getElementById('searchAllUsers').addEventListener('click', async event => {
  event.preventDefault();
  axios.get('https://crud-server-api-nodejs.herokuapp.com/users').then(function (response) {
...
document.getElementById('searchIndUser').addEventListener('click', async event => {
  ...
  axios.get(`https://crud-server-api-nodejs.herokuapp.com/user/${userId ? userId - 1 : 0}`)
    .then(function (response) {
      console.log(response.data)
...
document.getElementById('updateName').addEventListener('click', async event => {
  ...
  console.log(name)
  axios.put(`https://crud-server-api-nodejs.herokuapp.com/user/${userId}`, {
    "name": name,
  })
    .then(function (response) {
      // console.log(response)
...
document.getElementById('deleteIndUser').addEventListener('click', async event => {
  ...
  userId = Number(userId);
  axios.delete(`https://crud-server-api-nodejs.herokuapp.com/user/${userId}`)
    .then(function (response) {
      console.log(response.data)
